...
namespace crmSeries.Core.Tests.Features.Tasks
{
    ...
    public class BaseTaskDtoValidatorTests
    {
        ...
        [TestCase(-1, 1, false)]
        [TestCase(0, 1, false)]
        public void Validate_RelatedRecordId_ReturnsAppropriate(int relatedRecordId,
            int numberOfErrors,
            bool isValid)
        {
            //Arrange
            ...
            if (!result.IsValid)
            {
                Assert.AreEqual("'Related Record Id' must be greater than '0'.", result.Errors[0].ErrorMessage);
            }
        }
        ...
        [TestCase(TasksConstants.MaxRelatedRecordTypeLength, true)]
        [TestCase(TasksConstants.MaxRelatedRecordTypeLength + 1, false)]
        public void Validate_RelatedRecordTypeLength_ReturnsAppropriateValidation(
            int length,
            bool isValid)
        {
            //Arrange
            _baseTaskDto.RelatedRecordType = TestUtility.GenerateStringOfLength(length);
            ...
            if (!result.IsValid)
            {
                Assert.IsTrue(result.Errors[0].ErrorMessage.Contains(
                    TestUtility.MaxLengthError(
                        "Related Record Type", TasksConstants.MaxRelatedRecordTypeLength)));
            }
        }
